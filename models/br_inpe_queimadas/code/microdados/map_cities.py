...
def getCityUfField(data):
    data['cidade_uf'] = data['municipio'].str.title() + ' - ' + data['sigla_uf']
...
def getInpeCities():
    year_data =  pd.read_csv('./input/year_fire_data.csv')[['municipio','estado']]
    month_data = pd.read_csv('./input/month_fire_data.csv')[['municipio','estado']]
...
def getDirCities():
    dir_data = pd.read_csv('./extra/diretorio_municipios.csv') #Tabela de Municípios do Diretório da Base dos Dados
    dir_data['cidade_uf'] = dir_data['nome'] + ' - ' + dir_data['sigla_uf']
    return dir_data

def getSimilarCities(city, dir_data):
  results = difflib.get_close_matches(city, dir_data['cidade_uf'], cutoff=0.8)
  if len(results) > 0:
    return results[0]
  else:
    return None
...
def mapCityNames(inpe_data,dir_data):
    inpe_data['cidade_uf_dir'] = inpe_data['cidade_uf'].apply(lambda x: getSimilarCities(x,dir_data))
